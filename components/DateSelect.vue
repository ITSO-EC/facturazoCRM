      ...
      <div v-show="dropdownOpen" class="z-10 absolute top-full right-0 w-full bg-white border border-slate-200 py-1.5 rounded shadow-lg overflow-hidden mt-1">
        ...
        >
          ...
          <button
            ...
            :key="option"
            class="flex justify-center pr-8 items-center w-full hover:bg-slate-50 py-1 px-3 cursor-pointer"
            :class="options.indexOf(option) === selected && 'text-indigo-500'"
            @click="selected = options.indexOf(option); dropdownOpen = false; "
          >
            <svg class="shrink-0 mr-2 fill-current text-indigo-500" :class="options.indexOf(option) !== selected && 'invisible'" width="12" height="9" viewBox="0 0 12 9">
...
const clickHandler = ({ target }) => {
  if(dropdown.value.contains(target)) {
    emit('update-key',target.textContent);
  }
